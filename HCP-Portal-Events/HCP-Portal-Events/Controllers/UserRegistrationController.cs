...
namespace HCP_Portal_Events.Controllers
{
    ...
    public class UserRegistrationController : ControllerBase
    {
        ...
        public async Task<IActionResult> BookEvent(int userId, int eventId)
        {
            ...
            try
            {
                var user = await _unitOfWork.UserRepositories.GetUserByIdAsync(userId);
                if (user == null)
        ...
        // GET: api/userregistration/{userId}/previous
        [HttpGet("{userId:int}/previous")]
        public async Task<IActionResult> GetUserPreviousEvents(int userId)
        {
            if (userId <= 0)
            ...
            try
            {
                ...
                var events = await _unitOfWork.UserRepositories.GetUserPerviousEvents(userId);

                if (events == null || !events.Any())
                    return NotFound(new { Message = "No previous events found for this user." });
            ...
            catch (Exception ex)
            {
                return StatusCode(500, new
                {
                    Message = "An error occurred while fetching previous events.",
                    Details = ex.Message
                });
            }
        }

        // GET: api/userregistration/{userId}/upcoming
        [HttpGet("{userId:int}/upcoming")]
        public async Task<IActionResult> GetUserUpcomingEvents(int userId)
        {
            if (userId <= 0)
                return BadRequest(new { Message = "Invalid user ID." });
            ...
            try
            {
                ...
                var events = await _unitOfWork.UserRepositories.GetUserUpcomingEvents(userId);
        ...
        // GET: api/userregistration/{userId}/speciality
        [HttpGet("{userId:int}/speciality")]
        public async Task<IActionResult> GetUserSpecialityEvents(int userId)
        {
            if (userId <= 0)
                return BadRequest(new { Message = "Invalid user ID." });
            ...
            try
            {
                ...
                var events = await _unitOfWork.UserRepositories.GetUserSpecialityEvents(user);

                if (events == null || !events.Any())
                    return NotFound(new { Message = "No events found for user's speciality." });
            ...
            catch (Exception ex)
            {
                return StatusCode(500, new
                {
                    Message = "An error occurred while fetching speciality events.",
                    Details = ex.Message
                });
            }
        }
